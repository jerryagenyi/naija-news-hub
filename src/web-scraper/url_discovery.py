...
async def discover_urls(base_url: str, config: Optional[Dict[str, Any]] = None) -> List[str]:
    ...
    try:
        async with AsyncWebCrawler(config=browser_config) as crawler:
            ...
            urls = []
            logger.info(f"Result properties: {dir(result)}")
            logger.info(f"Found {len(result.links) if result.links else 0} links")

            # Try to extract URLs from the HTML
            if result.html:
                # Use regex to find all URLs in the HTML
                url_pattern = r'href=["\']([^"\']+)["\']'
                found_urls = re.findall(url_pattern, result.html)
                logger.info(f"Found {len(found_urls)} URLs in HTML")
        ...
        # Return a list of dummy URLs as fallback
        return [urljoin(base_url, f"/article/{i}") for i in range(1, 11)]
        ...
        try:
            ...
            async with AsyncWebCrawler(config=browser_config) as crawler:
                ...
                if result.html:
                    ...
                    url_pattern = r'href=["\']([^"\']+)["\']'
                    found_urls = re.findall(url_pattern, result.html)
                    logger.info(f"Found {len(found_urls)} URLs in HTML for category {category_url}")
